...
void EncoderNextOkButton::setupNextOkButtonEncoder(pinid_t pinNext, pinid_t pinOk, EncoderNextButtonListener *nextKeyListener, EncoderOkButtonListener *okKeyListener) {
    switches.setEncoder(new EncoderNextOkButton(pinNext, pinOk, (nextKeyListener == nullptr ? new EncoderNextButtonListener() : nextKeyListener), (okKeyListener == nullptr ? new EncoderOkButtonListener() : okKeyListener)));
}
